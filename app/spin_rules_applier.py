...
def get_spin_rules_path(provided_path=None):
    ...
    possible_paths = [
        "dataBase/spin-rules.ttl",  # When run from app directory
        "app/dataBase/spin-rules.ttl",  # When run from project root
        os.path.join(os.path.dirname(__file__), "dataBase/spin-rules.ttl"),  # Relative to this file
    ]
    
    for path in possible_paths:
        if os.path.exists(path):
            print(f"Found spin rules at: {os.path.abspath(path)}")
            return path
            
    raise FileNotFoundError(f"Could not find spin-rules.ttl in any of: {possible_paths}")
...
def run_update(update_string):
    """Executa uma query SPARQL UPDATE"""
...
def apply_spin_rules(spin_file_path="../dataBase/spin-rules.ttl", dry_run=False):
    """
    ...
    actual_path = get_spin_rules_path(spin_file_path)
    queries = extract_construct_queries_from_spin(actual_path)
